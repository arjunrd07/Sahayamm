...
function validatePaySlip(urlOrFilename: string): boolean {
...
describe("Borrower Compliance Verification & Advanced Repayment Math", () => {
  ...
  describe("Pay Slip Document Proof Validation", () => {
    it("accepts valid pay slip file uploads and URLs", () => {
      expect(validatePaySlip("salary_payslip_august_2026.pdf")).toBe(true);
      expect(validatePaySlip("payslip.jpg")).toBe(true);
      expect(validatePaySlip("https://storage.sahayam.app/payslips/doc123.png")).toBe(true);
    });

    it("rejects empty or invalid pay slip attachments", () => {
      expect(validatePaySlip("")).toBe(false);
      expect(validatePaySlip("   ")).toBe(false);
      expect(validatePaySlip("file_without_ext")).toBe(false);
    });
  });
